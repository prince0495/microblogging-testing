...
// import { Settings, Edit2, Users, UserCheck, AtSign, Calendar, FileText, Lock, Globe, Search, Link as LinkIcon, ShieldCheck, EyeOff, Bot, Server } from 'lucide-react';
// import axios from 'axios';
// import { PostCard } from './BlogsPage';

// // --- DUMMY DATA FOR API CALL SIMULATION ---
...
// // A single item in the Followers/Following list
...
// // Skeleton loader for the user list
...
// const ProfilePage = ({ account, userId }) => {
...
//   // This effect simulates fetching data when the view changes
//   useEffect(() => {
...
//         // In a real app: const response = await axios.get('/api/followers');
//         setFollowers(DUMMY_FOLLOWERS);
//       } else if (activeView === 'following') {
//         // In a real app: const response = await axios.get('/api/following');
...
//         if(timelineData?.statuses) {
//           setUserPosts(timelineData.statuses);
//           console.log(timelineData.statuses);
//         }
...
//   }, [activeView, followers.length, following.length]);
...
//                 <button className="flex-grow sm:flex-grow-0 px-5 py-2 rounded-full font-semibold bg-white text-black hover:bg-neutral-200 transition-colors duration-200 flex items-center justify-center">
...
//             )
//               //  : (              
//               //     <div className="text-center py-10 text-neutral-500">Your posts will be displayed here.</div>
//               //   )
//           }
...
//                 <div className="flex items-center justify-between"><span className="flex items-center"><Globe size={16} className="mr-2"/>Discoverable</span><span className={`font-bold ${account.discoverable ? 'text-green-400' : 'text-red-400'}`}>{account.discoverable ? 'Yes' : 'No'}</span></div>
//                 <div className="flex items-center justify-between"><span className="flex items-center"><Search size={16} className="mr-2"/>Search Engine Indexed</span><span className={`font-bold ${account.noindex ? 'text-red-400' : 'text-green-400'}`}>{account.noindex ? 'No' : 'Yes'}</span></div>
...
const EditProfileModal = ({ account, onClose, onSave }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Append fields_attributes in the required format
...
const ProfilePage = ({ account: initialAccount, userId }) => {
  ...
  const [isEditing, setIsEditing] = useState(false); // NEW state to control modal
  const [activeView, setActiveView] = useState('posts'); // 'posts', 'followers', or 'following'
  const [followers, setFollowers] = useState([]);
  const [following, setFollowing] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
        setIsLoading(true);
        try {
            // NOTE: This assumes your backend route is `/api/user/statuses`
            const timelineRes = await axios.get(`/api/user/statuses?userId=${userId}`);
  ...
  // NEW: Function to handle saving profile data
  const handleProfileUpdate = async (formData) => {
    try {
        // This is the call to your backend.
        // Your backend will then call the Mastodon API.
        ...
        // On success, update the local state with the new account data and close the modal
        console.log(response.data);
        
        if(response.data?.updatedAccount) {
          setAccount(response.data.updatedAccount); // Assuming your backend returns the updated account
        }
        else {
          console.log(response.data);
  ...
  return (
    <>
      {/* NEW: Render the modal when isEditing is true */}
